    ...
    <div className="w-full flex flex-col items-center gap-6">
      {pages.map((pageSlots, pageIdx) => (
        ...
        >
          <div>
            {/* Header Details */}
            <div className="grid grid-cols-2 gap-6 mb-3 text-[12.5px] font-medium border-b border-slate-300 pb-1.5">
              <div className="flex items-center">
                <span className="w-36 font-semibold">No Bukti transaksi :</span>
                <span className="font-bold text-slate-900">{noBuktiTransaksi || '-'}</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3.5">
              {pageSlots.map((item, index) => {
                ...
                return (
                  <div key={index} className="border-[1.5px] border-black flex flex-col justify-between" style={{ height: '375px' }}>
                    {/* Image Preview Container */}
                    <div className="flex-1 flex items-center justify-center p-2 overflow-hidden bg-slate-50/40">
                      {item.image ? (
                        <img
                          src={item.image}
                          alt={`Bukti ${globalIndex}`}
                          className="max-h-[315px] max-w-full object-contain shadow-sm"
                        />
                      ) : (
                    ...
                    {/* Description Bar */}
                    <div className="border-t-[1.5px] border-black px-2.5 py-1 text-[11px] bg-white flex items-start min-h-[28px]">
                      <span className="font-bold shrink-0 mr-1.5">Description :</span>
